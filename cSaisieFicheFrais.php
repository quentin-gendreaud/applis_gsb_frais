...
<!-- Partie Buggé -->	  
      ...
      <?php          
			...
			//req la requete avec les elements du forfait de ce visiteur avec ce mois là
			$req = obtenirReqEltsForfaitFicheFrais($mois,obtenirIdUserConnecte()); // a17 correspondant au code utilisateur	et le mois de mai
			//on execute la requete. idJeuEltsFraisForfait qui contient le resultat			
			$idJeuEltsFraisForfait=mysqli_query($connection,$req);
			//on recupère juste la ligne courante du resultat dans lgEltForfait			
			
			while ($lgEltForfait = mysqli_fetch_array($idJeuEltsFraisForfait))
			{	// tant qu'il reste des éléments dans la requete idJeuEltsFraisForfait faire
                $idFraisForfait = $lgEltForfait[0]; // stock l'id frais forfait de la BDD dans la variable
                $libelle = $lgEltForfait[1];	// de meme avec le libelle
                $quantite = $lgEltForfait[2];	// de meme avec la quantité
			...
			?>
            <p>
              <label for="<?php echo $idFraisForfait; ?>">* <?php echo $libelle; ?> : </label> <!--  Affiche le libelle de la catégorie  -->
              <input type="text" id="<?php echo $idFraisForfait; ?>" 
					...
					<!-- Affiche un cadre permetant la saisie des données pour les catégories concernées -->
            </p> 
            <?php 
			       
                //$lgEltForfait = mysqli_next_result($idJeuEltsFraisForfait);   plus utile avec le passage en mysqli.
            }
...
<?php          
		  ...
		  $connection=connecterServeurBD();
          $idJeuEltsHorsForfait = mysqli_query($connection,$req);
