...
function activerBD($idCnx) 
{
    ...
    $ok = mysqli_select_db($connection, $bd);
    return $ok;
}
...
function obtenirDetailVisiteur($idCnx, $unId) 
{
    ...
    $idJeuRes = mysqli_query($connection,$requete);  
    $ligne = false;     
    if ( $idJeuRes ) {
...
function ajouterFicheFrais($idCnx, $unMois, $unIdVisiteur) 
{
    ...
    $dernierMois = obtenirDernierMoisSaisi($connection, $unIdVisiteur);
	$laDerniereFiche = obtenirDetailFicheFrais($connection, $dernierMois, $unIdVisiteur);
	if ( is_array($laDerniereFiche) && $laDerniereFiche['idEtat']=='CR'){
		modifierEtatFicheFrais($connection, $dernierMois, $unIdVisiteur, 'CL');
	}
...
function verifierInfosConnexion($idCnx, $unLogin, $unMdp) 
{
    ...
    $idJeuRes = mysqli_query($connection,$req);
    $ligne = false;
    if ( $idJeuRes ) {
        $ligne = mysqli_fetch_assoc($idJeuRes);
        mysqli_free_result($idJeuRes);
